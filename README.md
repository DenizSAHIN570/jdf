...
toolchain (Fedora/Nobara, Arch, …) the AppImage step can fail — linuxdeploy's
bundled `strip` predates glibc's `.relr.dyn` relocation section and chokes on
every system library (`unknown type [0x13] section '.relr.dyn'`). Skip
